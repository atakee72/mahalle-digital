...
- **Photo preview in the compose form (fixed 2026-09-21, user report):** `ImageSlots.svelte` paints a filled tile with ONE `background` declaration (stripe tint + the photo). The tint was `{catColor}33` — fine for a hex colour, but `catColor` is always a CSS variable (`var(--cat-…, var(--k-wine))`), and `var(--k-wine)33` is invalid: the browser drops the WHOLE declaration, photo included. The preview had never shown since the kiosk form shipped; the upload itself worked. Now `color-mix(in srgb, {catColor} 20%, transparent)`. Two rules: never glue hex alpha digits onto a colour that may be a variable; and keep comments OUT of a Svelte `style="…"` attribute — a double quote inside breaks the markup (svelte-check: „Expected token =").
