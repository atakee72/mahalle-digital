...
4. Photo: `scratchpad/wahl2026/photos.mjs` — sharp, `.rotate()` first (bakes in EXIF orientation), max 2000 px, JPEG, NO metadata (one supplied photo carried a private description line and camera data). `coverCredit: "privat"` (press convention for a photo supplied by the person), and a per-photo `coverPosition` checked by eye on the article header — faces sit in the upper third. `cover` is optional; every surface handles a post without one. **A post that joins the group LATER (2026-09-19, successor candidate Hempel):** true `pubDate` (shown everywhere, `article:published_time`, month groups, masthead „zuletzt") plus the ORDER-ONLY frontmatter `sortDate` = the group's timestamp → `sortISO`, which `compareNewest()`/`isSimultaneous()` prefer. Without it the late post is simply the newest: lead card on the index and top of the tag page on election weekend. With it, it lines up by surname with an equal strip — used twice that night: Hempel (Die PARTEI) and Bristow (Volt); order now Bristow, Dehne, Haghanipour, Hempel, Lüders, Mende. A square studio portrait on a plain wall (Bristow) takes the same wide-composite recipe, with the fill left at the wall's own brightness so it reads as one photo (`scratchpad/wahl2026/bristow-photo.mjs`); a photographer's credit goes into `coverCredit` as supplied („© Evelin Frerk"). Numbered paragraphs in a verbatim text are written `1\.` — Markdown would otherwise turn them into an `<ol>` whose numbers are not text. Bystanders in a supplied photo (here a police officer at the edge) are cropped out before the composite (`scratchpad/wahl2026/hempel-photo.mjs`). Copy that states a COUNT of posts goes stale — the tag page's preview text is number-free since then; the intro post's count is editorial and the author's to update. **A PORTRAIT-format photo never goes in as-is**: every cover box on the blog is wide (article hero ~2.8:1, index strip, tag card), so `object-fit: cover` crops it to the eyes (Lüders, 427×640, 2026-09-18). Build a wide composite instead — the portrait sharp at full height, centred over a blurred, darkened fill of itself (`scratchpad/wahl2026/lueders-photo.mjs`: 1600×900 cover + 1200×630 link-preview card) — then set `coverPosition` so the whole head shows.
