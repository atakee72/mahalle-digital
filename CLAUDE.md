...
- `rateLimits` - Fixed-window rate-limit buckets (`{ key: '<baseKey>#<windowId>', baseKey, count, expiresAt }`, TTL index). Used by login lockout (5 fails/15min), the News link preview (30/h per member, `newsprev:<userId>` — SSRF-hardened `GET /api/news/preview`, see `src/pages/api/news/CLAUDE.md`), forgot-password (20/h IP + 3/h email, silent), register (40/h IP + 3/h email — IP gates are ROOM-sized since 2026-09-14: a tablet, hotspot or venue Wi-Fi is one IP; the email bucket is the per-person brake), resend-verification (10/h user). See `src/lib/auth/rateLimit.ts`; indexes via `scripts/create-auth-indexes.ts`.
