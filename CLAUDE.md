...
Both bars take the **section colour** since 2026-09-22 (user decision „option A — one colour for both bars"): Forum a lighter burgundy (`--k-wine-deep` `#a1415f` since 14:49; the legacy `--color-secondary` `#814256` held for 15 minutes, then he asked for „a tick lighter, a bit livelier“ and picked the middle of three — user 2026-09-22 14:13: the kiosk wine `#b23a5b` read loud as a full bar; bars only, forum kickers/„+"/shadows stay `#b23a5b`), Kalender teal, Kurier ink, Kiez-Daten moss, Blog rust; **Markt stays ochre** (user: „its title also includes that colour"), Profil/Auth and pages without a `data-page` stay ochre; the admin masthead stays plum. Mechanism: `--k-bar` / `--k-bar-fg` (+ `--k-bar-hover`, `--k-bar-disc-ring`, `--k-bar-dim`) keyed off `<body data-page>` in `tokens.css`, read by `KioskNav.svelte` — no page knowledge in the island. (Until 2026-09-22 both bars were ochre everywhere.) Bar rhythm: 54 px on phones / 67 px desktop, title-block kickers sit 20/24 px under the masthead rule on every surface. Anatomy of the DE/EN pill and the paper avatar disc: `src/components/forum/kiosk/CLAUDE.md` → "Ochre masthead". Since 2026-09-19 the top bar hides on scroll down / returns on scroll up below `lg` (bottom nav stays fixed; moves via `top`, never `transform`; publishes `--k-mast-offset`) — details in the same area file, „Hide on scroll".
...
|---|---|---|---|
| Forum | wine | `text-wine` (`#b23a5b`) | lighter burgundy `#a1415f` (`--k-wine-deep`, was `#814256` for 15 min) |
