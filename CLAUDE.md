...
- `savedPosts` - User bookmarks for forum posts (userId + postId pairs, server-side persistence). Feed cards show a per-post save COUNT (read-time `$group`, `attachSavedCounts()` in `topicsQuery.ts`) since 2026-09-11 — never store a counter on the post.
