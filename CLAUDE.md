...
- **Username validation**: `checkNameProfanity()` at registration — runs all 3 blocklists (word-boundary + substring check for concatenated profanity like "PenisPenisPenis"), then `moderateText()` + `checkSpamWithGPT()` via OpenAI as safety net for creative spellings and hate speech. **An OpenAI failure must never refuse a name** (fixed 2026-09-18): those two functions do not throw, they RETURN the fail-safe result (`pending_review` + `moderation_error`), which is right for a post (it waits in the queue) but a name has no queue — the old `decision !== 'approved'` test refused every signup with „Name contains inappropriate content" during an outage. `shortTextVerdict()` (`src/lib/shortTextVerdict.ts`, pure, tested) skips pure fail-safe results and refuses only on a real flag; the same path guards the profile motto. General rule: any NEW caller of the moderation functions that has no review queue must handle the fail-safe shape explicitly — a `try/catch` around them catches nothing.
...
### Blog ("Die Beilage")
**Kiosk migration complete** (July 2026): `/blog` moved from the legacy dark-glass blog (`BlogBaseLayout.astro` + `BlogSearch`/`ImageGallery`/`TagCloud`/`TagBarMobile`/`BlogCard` — all deleted, no trace remains) to the kiosk design system — `src/pages/blog/*` + `src/layouts/blog/*` (`StandardLayout`/`HeroLayout`/`GalleryLayout`) + `src/components/blog/kiosk/` (Svelte islands, rust accent), SSR on every route (no prerendering — see the area file for why). Full notes: `src/components/blog/CLAUDE.md` (architecture, island split, `beilage.ts` helpers, Druckbogen print recipe, draft gating, states matrix) load when working in that subtree. Since 2026-09-18 (election guest posts): one shared post order `compareNewest()` with a file-id tiebreak, no lead card or photo strip for posts published in the same second, real author names on cards, and a verbatim guest-post recipe (docx → MDX, smartypants trap, metadata-free photos) — section „Simultaneous posts" there. Since 2026-09-19 a post that joins such a group LATER keeps its true `pubDate` and carries the order-only frontmatter `sortDate` (→ `sortISO`, preferred by `compareNewest()`/`isSimultaneous()`), so it lines up inside the group by surname instead of taking the lead card as the newest post. Blog posts are repo files, so publishing one is a deploy.
...
- `schillerkiez_air_log` - BLUME air readings for station mc042, appended every 30 min by the GitHub-Actions-triggered `GET /api/cron/log-air` (Bearer `CRON_SECRET`, fail-closed 503 when unset). **The workflow's `APP_ORIGIN` must be the canonical domain** (`https://mahalle.digital`): the old `*.vercel.app` origin 308-redirects there and `curl` without `-L` scores that a failure — which silently killed the logger for 2 days after the Aug 2026 domain cutover. `-L` is NOT the fix (curl drops `Authorization` across hosts → 401). Nothing alerts on this by itself — the request dies at the edge, so neither Sentry nor the app's degradation captures fire, and only the Actions tab goes red. **Mitigated Aug 2026** by `checkAirLoggerFreshness()` (`src/lib/kiez/airFreshness.ts`), called from the Vercel-native news cron: it Sentry-alerts when this collection has had zero rows in 24h. One doc per BLUME measurement `ts` (unique index; duplicates dropped), with Europe/Berlin `day` key and LQI + pollutant grades. **BLUME's „no measurement" is the NUMBER `-1`, not null** (seen 2026-09-19 22:00, every component): `fetchMc042()` normalises it to `null` via `normalizeBlumeComponents()` (`src/lib/kiez/blume.ts`, `isValidGrade` = integer 1–5) — the single entry point, so the live route answers 502 („station silent" strip state), the logger logs nothing (the gap is the honest record), and print/landing never see it. Before the fix `-1` was shown on the strip and on the public landing row, stored as a reading and averaged into the day (`lqiMean -0.1`). `buildDailyRollup()` ignores invalid stored rows, `recomputeDailyRollup()` deletes a rollup that has no valid reading left, `getAirHistory().lastReading` skips invalid rows. Repair of rows stored before the fix: `scratchpad/repair-air-sentinel.mts` (dry-run default, user-run). Tests: `src/lib/kiez/airSentinel.test.ts`. **Substitute station (2026-09-20, user decision after mc042 went down as the only one of Berlin's 15 stations):** while mc042 has no LQI, the LIVE strip (`/api/kiez-air` → `pickAirSource()`) shows `mc221` Karl-Marx-Straße 38 — 0.9 km from Herrfurthplatz, the nearest live station, but a TRAFFIC station (kerb of a main road, no O₃/CO). Always labelled („ERSATZWERT · STATION KARL-MARX-STRASSE · MC221") with the caveat that values there are usually a little higher than in residential streets (`kiez.strip.substitute.*`, response field `substitute`). NEVER logged and never in the 7-day history — the logger, the print page and the landing row stay mc042-only (the landing row stays muted while mc042 is down). No third station is ever used. The morning freshness alarm (`airFreshness.ts` + pure `silenceKind()`) now asks the feed: empty window + mc042 silent ⇒ its own WARNING issue „station mc042 silent … (logger healthy)" (leave it unresolved while the station is down, or it reopens every morning); empty window + mc042 reporting, or BLUME unreachable ⇒ the old ERROR „air logger silent". Tests: `src/lib/kiez/airSubstitute.test.ts`. Hourly rows pruned after 90 days.
...
SENTRY_AUTH_TOKEN=       # SERVER-ONLY secret. Never exposed to the client. Powers /api/admin/errors + sourcemap uploads (astro.config.mjs). READ-scoped: issue writes (resolve/ignore) return 403 — resolve issues in the Sentry UI (select-all → Resolve), verified 2026-09-16. Stats API: `interval=1d` (a 30d interval is rejected).
...
Both bars take the **section colour** since 2026-09-22 (user decision „option A — one colour for both bars"): Forum a lighter burgundy (`--k-wine-deep` `#95425d` since 14:53 — the legacy `--color-secondary` `#814256` was too dull as a full bar, `#a1415f` too close to the kiosk wine; he chose the step between after seeing both live — user 2026-09-22 14:13: the kiosk wine `#b23a5b` read loud as a full bar; bars only, forum kickers/„+"/shadows stay `#b23a5b`), Kalender teal, Kurier ink, Kiez-Daten moss, Blog rust; **Markt stays ochre** (user: „its title also includes that colour"), Profil/Auth and pages without a `data-page` stay ochre; the admin masthead stays plum. Mechanism: `--k-bar` / `--k-bar-fg` (+ `--k-bar-hover`, `--k-bar-disc-ring`, `--k-bar-dim`) keyed off `<body data-page>` in `tokens.css`, read by `KioskNav.svelte` — no page knowledge in the island. (Until 2026-09-22 both bars were ochre everywhere.) Bar rhythm: 54 px on phones / 67 px desktop, title-block kickers sit 20/24 px under the masthead rule on every surface. Anatomy of the DE/EN pill and the paper avatar disc: `src/components/forum/kiosk/CLAUDE.md` → "Ochre masthead". **Frames + bevel since 2026-09-22 evening:** `--k-bar-pill-border` (ink by default, paper on the coloured bars) frames the DE/EN pill, the avatar and the bell; the logo disc's ring is ink on the ochre bars and paper elsewhere; the SELECTED desktop tab is always framed in paper; and all of them — plus the bottom nav's active chip and the floating „+" — carry an inset bevel whose visible line sits on the right. Recipes and the reason (a rendering seam the user liked) in the area file → „frames and bevelled edges". Since 2026-09-19 the top bar hides on scroll down / returns on scroll up below `lg` (bottom nav stays fixed; moves via `top`, never `transform`; publishes `--k-mast-offset`) — details in the same area file, „Hide on scroll".

### Page inset rule (kiosk, 2026-09-19)
Every main section's page-level wrappers use ONE side inset: **16 px on phones · 36 px from `md` · 40 px from `lg`** — `px-4 md:px-9 lg:px-10` (margins: `mx-4 md:mx-9 lg:mx-10`; in plain CSS: 16 / `min-width: 768px` 36 / `min-width: 1024px` 40). Until that day each section set its own: forum 16/32/40, calendar 16/36/40, news 16/36/36, Markt 20/20/36 with its lead card at 12 (heading and cards did not share an edge), Kiez-Daten 20 (its station strip 18)/20/36 — the user saw it as „different indents". There is no shared wrapper component; when adding a page-level block, copy the three classes. Deliberate exceptions: the calendar month grid runs edge to edge on phones, the blog's centred masthead and 24 px column, the News nameplate („Schillerkiez Kurier", inset 36 px at every width), the Markt pager (centred). Measure with `scratchpad/indents.cjs` (prod, heading + first block, four widths) and `scratchpad/indent-edges.cjs` (every boxed block's left/right edge; env `PROBE_WIDTHS`). **Page width (decided 2026-09-20, „centre all three"):** every main section sits in ONE centred 1280 px column, like the masthead (`max-w-7xl`). Forum carries it on its `<main>`, Kiez-Daten on the orchestrator root, and calendar / News / Markt get it from a plain `<div class="mx-auto w-full max-w-[1280px]">` around the island in their `.astro` page (a plain block — no transform/filter — so fixed and sticky children keep working). Before that day those three had no maximum width: on a 1600 px screen their headings sat at 36–40 px while logo, forum and Kiez-Daten sat at ~200. Measured after: all five headings at 40 / 200 / 360 px on 1280 / 1600 / 1920 px screens, phones unchanged. A new main section must bring the same wrapper.
...
When migrating a surface into kiosk, swap kicker + italic-accent text to the page's color. **Don't touch:** live-now indicators, weekend day labels, required-field asterisks, compose step numbers (`01`, `02`, …), CTA wine-shadows, modal wine-shadows — those are semantic/sticker accents, not brand accent, and stay wine across all surfaces. **Exception since 2026-09-22 (user decision): the calendar's today indicator (the filled day-number disc + the „HEUTE" label) is teal, matching its section accent, since the calendar bars went teal** — every other surface's today-equivalent stays wine. **Exception since 2026-09-20 (user decision): the floating „+" button on phones takes the page colour** — forum wine, market a DARKER ochre `#b97a1a` with a paper plus since 2026-09-22 (user trial: plain ochre `#e8a53a` had no contrast for a paper plus and vanished against the ochre bottom bar), calendar teal, News ink (with the wine print shadow); all four surfaces have one now, see `src/components/forum/kiosk/CLAUDE.md` → „Floating „+" on phones".
...
### First database connect on a cold instance: one retry, never a Sentry filter (2026-09-20)
`connectDB()` wraps `getClientPromise()` in `connectWithRetry()` (`src/lib/mongoRetry.ts`, pure, tested): a `MongoServerSelectionError` / `MongoNetworkError` / `MongoNetworkTimeoutError` from the INITIAL connect is retried once — the failed attempt is already un-cached and closed, so the second call builds a fresh client. Reason: Sentry MAHALLE-PROD-2 reopened with exactly one event in the minute after a deploy, three times in two days; every stack was `MongoClient.connect → … → Topology.selectServer` (cold start, no pool), and the very next request always succeeded. Deliberately NOT filtered out of Sentry: a real outage has the same signature and must stay loud — it still throws (last error, two attempts ≈ 20 s instead of 10 s) and the middleware captures it. Verified with the real driver (`scratchpad/mongo-retry-live.mts`: unreachable host → dev DB, failed client closed, 0 unhandled rejections; full outage → error after 2 attempts). **The default export `clientPromise` is taken ONCE at module load and never replaced** — if that first connect fails it stays rejected for the life of the instance (the un-cache only frees the globalThis slot), so every `await clientPromise` there keeps failing while `connectDB()` recovers. Since 2026-09-22 (Sentry: the three post-fix blips were all `GET /api/profile/tour`, which awaited it) login (`authorize`), the JWT recheck, `register`, `users/update` and `profile/tour` use `connectDB()`; the Auth.js adapter is the only remaining consumer and is never called with Credentials + JWT. New code: always `connectDB()`, never the default export. Full history: `docs/runbooks/mongo-region-incident.md`.
...
- **Client `beforeSend` noise filter (2026-09-20):** `sentry.client.config.ts` drops what `isClientNoise()` (`src/lib/sentry/clientNoise.ts`, pure, tested with the exact strings from the board) recognises — cancelled view transitions (`AbortError`; `InvalidStateError` ONLY when the text is about transitions / a hidden document), React's „This root received an early update, before anything was able hydrate" fallback notice, and `null (reading 'body')` ONLY when a stack frame is Astro's `ClientRouter` (a page open during a deploy; from our own files the same message is a real bug). They reopened issues after every deploy and every backgrounded tab. Keep it that narrow — never add a pattern for something a member could actually see.
- **`beforeSend` transient filter** (server config): drops OpenAI 429/rate-limit noise, `AbortError`, and transient `MongoNetworkError` timeouts before they ship — the moderation pipeline's per-submission OpenAI calls would otherwise let a provider incident burn the monthly cap. **Also drops `event.environment === 'development'` (2026-09-03, `9c6434f8`)**: the server-side `SENTRY_DSN` stays active in local dev on purpose (diagnostics), but its events — including unhandled promise rejections from throwaway git-worktree dev servers (e.g. Astro's route-manifest reader ENOENT-ing on files a `git reset` removed mid-run) — were landing on the prod board and burning the cap (was the #1 issue, 62 events, before the filter). `development` is dropped, not "non-production", so **preview + production still report** — preview is where the silent-degradation tripwires legitimately fire. This is the server-side twin of the client-side `PUBLIC_SENTRY_DSN`-commented-out fix.
