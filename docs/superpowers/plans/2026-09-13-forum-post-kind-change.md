...
- **Audit 2026-09-13 (pre-execution):** switched the kind cards from `role="radio"` to the repo's `aria-pressed` toggle convention (like/bookmark buttons, DE/EN pill) to avoid new svelte-check a11y noise; added T6 because the user tested likes/saves/comments on the real post — likes and comments needed nothing, saves needed the Bookmarks join. Verified: no `noUnusedLocals` in tsconfig (the `_`-prefixed destructure in `buildMovedDoc` is fine), `topics/create` responds `{ topic: { _id } }`, the forum index builds a fresh QueryClient per mount and seeds from SSR (`initialDataUpdatedAt: Date.now()`), so after the hard navigation there is no stale-cache hazard (the persisted React client does not cover it).
